...
export class WebRtcClient {
    ...
    http: Http;
    sigServeUrl = 'http://localhost:1337/';
    serverName: string;
    processName: string;
    ...
    constructor(serverName: string) {
        ...
        // this.http.get(this.sigServeUrl + "create/" + this.serverName, (response) => {
        //     this.rtcPeerConnection.setRemoteDescription(JSON.parse(JSON.parse(response).desc));
        //     this.rtcPeerConnection.createAnswer().then(
        //         (desc) => {
        //             this.rtcPeerConnection.setLocalDescription(desc);
        //             // this.connectSigServe(JSON.stringify(desc));
        //         },
        //         (err) => {
